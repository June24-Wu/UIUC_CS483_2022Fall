...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  int tx = threadIdx.x; int bx = blockIdx.x ; int i = bx * blockDim.x + tx;

  if (i < dim){
    int row_size = matRows[i];
    float ans = 0;
    for (int j = 0 ; j < row_size; j++){
      int idx = matColStart[j] + i;
      int col = matCols[idx];
      ans += vec[col] * matData[idx];
    }
    __syncthreads();
    out[matRowPerm[i]] = ans;
  }
  return ;
}
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 gridDim(ceil(1.0*dim / BLOCK_SIZE),1,1);
