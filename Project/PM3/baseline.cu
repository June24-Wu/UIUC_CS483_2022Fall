...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    int GridDim = ceil(1.0 * Width / TILE_WIDTH);
    int m = blockIdx.x; 
    int h = (blockIdx.y / GridDim) * TILE_WIDTH + threadIdx.y; 
    int w = (blockIdx.y % GridDim) * TILE_WIDTH + threadIdx.x;
    int b = blockIdx.z;
    ...
    for (int c = 0; c < Channel;c++){
        for (int k1 = 0;k1 < K;k1++){
            for (int k2 = 0; k2 < K ;k2++){
                ans += in_4d(b,c,h+k1,w+k2) * mask_4d(m,c,k1,k2);
            }
        }
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    int W_grid = ceil(1.0 * Width / TILE_WIDTH);    // Number of horizontal tiles for output maps
    int H_grid = ceil(1.0 * Height / TILE_WIDTH);    // Numer of vertical tiles for output maps
    int Y = W_grid * H_grid;
